function GetPods()
  return vim.fn.system(
    "echo -n $(kubectl get pods --no-headers  -o custom-columns=\":metadata.name\")")
end
...
local function change_namespace()
  ExecuteK8sFunctionFromSelect({
    select_choices = vim.fn.system(
      "echo -n $(kubectl get namespace --no-headers  -o custom-columns=\":metadata.name\")"),
      ...
      -- which key integration
      {
        ...
        opts = function(_, opts)
          if require("lazyvim.util").has("noice.nvim") then
            opts.defaults["<leader>k"] = { name = "+kubernetes" }
            opts.defaults["<leader>kg"] = { name = "+get" }
            opts.defaults["<leader>kl"] = { name = "+logs" }
            opts.defaults["<leader>klg"] = { name = "+grep" }
          end
        end,
      },
    },
